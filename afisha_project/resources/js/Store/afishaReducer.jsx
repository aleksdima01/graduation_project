...
export const fetchAfisha = createAsyncThunk(
    ...
    async (data, thunkApi) => {
        ...
        try {
            const response = await fetch(`http://192.168.101.126:8081/api/getfetchinfo?lang=&fields=id,dates,publication_date,title,short_title,place,location,images,site_url&expand=&order_by=-publication_date&text_format=&ids=&page=${data.page}&page_size=21&location=${data.city}&actual_since=${Date.now() / 1000}`, {
                method: "GET",
            });
            if (!response.ok) {
